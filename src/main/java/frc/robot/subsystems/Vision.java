...
public class Vision extends SubsystemBase {
    ...
    private void getNewVisionDataFromCameraX(int x) {
        ...
        if (chassis.getVelocity() <= maxValidVelcity) {
            ...
            Pose2d visionRobotToFieldPose2d = cameraToRobotCenter
                    .plus((aprilTagToFieldPose2d).minus(cameraToAprilTagPose2d)); // !
            VisionData newVisionData = new VisionData(visionRobotToFieldPose2d,
                    latestResult.getTimestampSeconds() - (latestResult.getLatencyMillis() / 1000));
        ...
        if (latestResult.hasTargets()) {
            ...
            if (bestTarget != null) {
                ...
                Pose2d cameraToAprilTagPose2d = new Pose2d(
                        bestTarget.getBestCameraToTarget().getTranslation().toTranslation2d(),
                        bestTarget.getBestCameraToTarget().getRotation().toRotation2d());
                Translation2d visionRobotToFieldTranslation2d = (aprilTagToFieldPose2d.getTranslation())
                        .minus(cameraToAprilTagPose2d.getTranslation()).plus(cameraToRobotCenter.getTranslation());
                Rotation2d visionRobotToFieldRotation2d = cameraToAprilTagPose2d.getRotation()
                        .plus(cameraToRobotCenter.getRotation());
                ...
                VisionData newVisionData = new VisionData(
                        new Pose2d(visionRobotToFieldTranslation2d, visionRobotToFieldRotation2d),
                        latestResult.getTimestampSeconds() - (latestResult.getLatencyMillis() / 1000));
                if (newVisionData != null && newVisionData.pose != null) {
                    if ((newVisionData.pose).getTranslation()
                            .getDistance(aprilTagToFieldPose2d.getTranslation()) > maxDistanceOfCameraFromAprilTag)
